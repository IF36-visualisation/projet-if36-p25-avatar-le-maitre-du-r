...
Dans le cadre de notre projet, nous avons choisi d'étudier divers jeux de données autour du catalogue de jeux Steam. Les datasets utilisés lors de ce projet sont les suivants : - [Dataset de NewbieIndieGameDev datant d'Octobre 2024](https://github.com/NewbieIndieGameDev/steam-insights) (140 000 jeux recensés)
...
tableau_stat <- df_long %>%
  ...
  mutate(variable = case_when(
    ...
    variable == "metacritic" ~ "Note Metacritic",
    variable == "avis" ~ "Nombre d'avis",
    TRUE ~ variable
  ))

# Affichage
kable(tableau_stat, col.names = c("Variable", "Moyenne", "Ecart-Type"), caption = "Statistiques descriptives principales", digits = 2)
```

Notes :

- La moyenne des notes Metacritics ne concerne que les jeux ayant cette note
...
- Nous avons omis la proportion d'avis négative dans le tableau car nous obtenons juste le résultat inverse que pour celui du positif (corrélation négative)
...
## Types des jeux en lignes sur Steam (Hoang-Viêt LE)
...
C'est sur la plateforme Steam que l'on a vu naître les premiers jeux en ligne et le développement de celui-ci avec des jeux mythiques comme Counter Strike, Team Forteress, etc... D'autres types de jeux sont aussi devenus populaires au cours du temps comme les MMORPG, les Battle-Royale, etc... Nous voulons voir les tendances actuelles et quels types de jeux attirent principalement les joueurs de Steam.
...
Une définition de jeu en ligne sera un jeu vidéo dont l'expérience se repose sur l'intéraction entre plusieurs joueurs en temps réel. On distingue différents types de jeu en ligne :
