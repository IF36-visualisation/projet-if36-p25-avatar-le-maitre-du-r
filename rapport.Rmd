...
top50_cat <- cat_pop_long %>% slice_max(total_owner_min, n = 50)
...
# On suppose que 'tags' contient app_id et tag (un tag par ligne)
# Et que 'clean_games' contient release_date (format AAAA-MM-JJ) et app_id

# Joindre les tags et les dates de sortie
...
# Compter le nombre de jeux par tag et par année
genre_year <- games_tags %>%
  group_by(year, tag) %>%
  summarise(n = n(), .groups = "drop")

# Choisir les 6 tags les plus fréquents (pour la lisibilité du graphique)
top_tags <- genre_year %>%
  group_by(tag) %>%
...
genre_year_top <- genre_year %>% filter(tag %in% top_tags)

# Graphique d'évolution des principaux genres/tags
ggplot(genre_year_top, aes(x = year, y = n, color = tag)) +
  geom_line(linewidth = 1.1) +        # <-- utilise linewidth
  geom_point(size = 1.6) +
  labs(
    title = "Évolution du nombre de jeux sortis par genre/tag (top 6)",
    x = "Année de sortie",
...
ggplot(genre_year_all_top, aes(x = year, y = n, color = tag)) +
  ...
  geom_point(size = 1.6) +
  labs(
    title = "Évolution du nombre de jeux sortis par tag (tous jeux, top 10 tags)",
    x = "Année de sortie",
    y = "Nombre de jeux sortis",
    color = "Tag"
  ) +
...
# Graphique avant 2015
ggplot(genre_year_pre2015_top, aes(x = year, y = n, color = tag)) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 1.6) +
  labs(
    title = "Évolution des tags pour les jeux sortis avant ou en 2015 (top 10 tags)",
    x = "Année de sortie",
    y = "Nombre de jeux sortis",
    color = "Tag"
  ) +
...
## 3. Jeux sortis après 2015 (top 10 tags)
...
# Graphique après 2015
ggplot(genre_year_post2015_top, aes(x = year, y = n, color = tag)) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 1.6) +
  labs(
    title = "Évolution des tags pour les jeux sortis après 2015 (top 10 tags)",
    x = "Année de sortie",
    y = "Nombre de jeux sortis",
    color = "Tag"
  ) +
