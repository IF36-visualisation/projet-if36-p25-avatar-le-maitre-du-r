...
```{r game_released_by_year, results='hide'}
...
- On observe une **augmentation brusque** en **2006**. Ceci est le résultat du programme **Steam Greenlight**, un service qui permettait aux développeurs de soumettre leur jeu pour que la communauté Steam puisse voter pour ceux qui vont intégrer le catalogue de la plateforme.
Lancé en 2004, ce service va permettre à plus en plus de jeux de pouvoir intégrer le catalogue Steam.


### Sources des explications :

- Lien vers l'histoire de Steam  : [Lien Wikipédia de Steam](https://en.wikipedia.org/wiki/Steam_\(service\))
- Interview de Gabe Newell sur Steam Greenlight  : [Interview sur Steam Greenlight en 2006](https://www.gamedeveloper.com/business/for-gabe-newell-greenlight-is-just-a-stepping-stone-to-a-bigger-endgame)
...
```{r popular_game_chart, results='markup'}
...
```{r popular_game_june_2025_chart, results='markup'}
...
```{r Ubisoft vs Supergiant vs FromSoftware, results='markup'}
...
```{r AAA genres combinaisons, results='markup'}
...
```{r Indie genres combinaisons, results='markup'}
